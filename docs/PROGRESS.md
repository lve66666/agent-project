...
| P6 演示与提交 | 进行中 | `tools/verify.ps1`：20 项通过；模拟 HTTP CLI 端到端通过；README.txt 699 字符 | 用真实 API 在 demo_project 录制视频并填写仓库地址 |

## 本次计划

增加 `search_text` 工具：支持关键词/正则、结果上限、工作区边界和二进制/超大文件跳过；待配置公开仓库、运行一次真实 API 演示、录制视频并生成姓名.zip。
